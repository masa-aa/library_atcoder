...
for i in range(n):
    ...
    s -= x
    t -= x
    # Trueが挿入イベント, Falseが削除イベント
    eves.append((s, True, x))
    eves.append((t, False, x))

# イベントを時系列でソートする
eves.sort(key=lambda x: x[0])
# print(*eves, sep='\n')
...
for i in range(q):
    # d[i]以下のイベントを全て処理する(stにadd, remove)
    while k < len(eves) and eves[k][0] <= d[i]:
        if eves[k][1]:
            st.add(eves[k][2])
        else:
            st.remove(eves[k][2])
        k += 1
    # 引っかかるところはSortedListの最小値
